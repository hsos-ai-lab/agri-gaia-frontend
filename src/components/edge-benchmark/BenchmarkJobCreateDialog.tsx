...
export default function ({ onCreate, handleClose }: IBenchmarkJobCreateProps) {
    const keycloak = useKeycloak();

    const [benchmarkConfig, setBenchmarkConfig] = useState<Record<string, any>>({
        schema: {
            description: 'Configure and run a benchmark job on selected devices.',
            type: 'object',
            properties: {
                devices: {
                    title: 'Devices',
                    ...
                    description: 'List of selected edge devices to benchmark.',
                    minItems: 1,
                },
                inference_client: {
    ...
    const onFormSubmit = (form: any) => {
        ...
        setIsCreating(true);
        setBenchmarkConfig({ ...benchmarkConfig, values: form.formData });
        console.log('BenchmarkConfig:', benchmarkConfig);
    };

    return (
        <Dialog open={true} onClose={onDialogClose} fullWidth maxWidth="xs">
            <DialogTitle>Configure Benchmark Job</DialogTitle>
            <DialogContent>
